...
bool convert<geometry_msgs::TransformStamped>::decode(const Node& node, geometry_msgs::TransformStamped& rhs)
{
  ...
  if (isNode(node["child_frame_id"]))
    rhs.child_frame_id = node["child_frame_id"].as<std::string>();

  if (isNode(node["transform"]))
...
bool convert<geometry_msgs::Pose>::decode(const Node& node, geometry_msgs::Pose& rhs)
{
  ...
  if (isNode(node["position"]))
    rhs.position = node["position"].as<geometry_msgs::Point>();

  if (isNode(node["orientation"]))
...
bool convert<moveit_msgs::CollisionObject>::decode(const Node& node, moveit_msgs::CollisionObject& rhs)
{
  ...
  geometry_msgs::Pose pose_msg = poseEigenToMsg(pose);
  ...
  if (isNode(node["primitives"]))
  {
    ...
    for (auto& primitive_pose : rhs.primitive_poses)
      primitive_pose = poseEigenToMsg(pose * poseMsgToEigen(primitive_pose));
    // #endif
  }

  if (isNode(node["meshes"]))
  {
    ...
    for (auto& mesh_pose : rhs.mesh_poses)
      mesh_pose = poseEigenToMsg(pose * poseMsgToEigen(mesh_pose));
    // #endif
  }

  if (isNode(node["planes"]))
  {
    ...
    for (auto& plane_pose : rhs.plane_poses)
      plane_pose = poseEigenToMsg(pose * poseMsgToEigen(plane_pose));
    // #endif
  }
...
bool convert<octomap_msgs::Octomap>::decode(const Node& node, octomap_msgs::Octomap& rhs)
{
  ...
  if (isNode(node["binary"]))
    rhs.binary = nodeToBool(node["binary"]);

  if (isNode(node["id"]))
    rhs.id = node["id"].as<std::string>();

  if (isNode(node["resolution"]))
...
bool convert<moveit_msgs::Constraints>::decode(const Node& node, moveit_msgs::Constraints& rhs)
{
  ...
  if (isNode(node["joint_constraints"]))
    rhs.joint_constraints = node["joint_constraints"].as<std::vector<moveit_msgs::JointConstraint>>();

  if (isNode(node["position_constraints"]))
    rhs.position_constraints = node["position_constraints"].as<std::vector<moveit_msgs::PositionConstraint>>();

  if (isNode(node["orientation_constraints"]))
    rhs.orientation_constraints = node["orientation_constraints"].as<std::vector<moveit_msgs::OrientationConstraint>>();

  if (isNode(node["visibility_constraints"]))
...
bool convert<moveit_msgs::PositionConstraint>::decode(const Node& node, moveit_msgs::PositionConstraint& rhs)
{
  ...
  rhs.link_name = node["link_name"].as<std::string>();
  if (isNode(node["target_point_offset"]))
...
bool convert<moveit_msgs::RobotTrajectory>::decode(const Node& node, moveit_msgs::RobotTrajectory& rhs)
{
  ...
  if (isNode(node["joint_trajectory"]))
    rhs.joint_trajectory = node["joint_trajectory"].as<trajectory_msgs::JointTrajectory>();

  if (isNode(node["multi_dof_joint_trajectory"]))
    rhs.multi_dof_joint_trajectory = node["multi_dof_joint_trajectory"].as<trajectory_msgs::MultiDOFJointTrajectory>();
